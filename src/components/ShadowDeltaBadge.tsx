...
const ShadowDeltaBadge: React.FC<ShadowDeltaBadgeProps> = ({
  ...
  if (!isActive && rounded > 0) {
    return (
      <div
        ...
        title="Extra polish was detected, but confidence is still warming up. Final score stays on the safer baseline."
      >
        Extra Polish (Paused)
      </div>
    );
  ...
  const styles = isActive
    ? rounded >= 3
      ? { bg: 'bg-emerald-500/20', text: 'text-emerald-400', label: `Extra Polish +${rounded}` }
      : rounded <= -1
        ? { bg: 'bg-amber-500/20', text: 'text-amber-400', label: `Extra Polish ${rounded}` }
        : { bg: 'bg-slate-500/20', text: 'text-slate-300', label: `Extra Polish ±${Math.abs(rounded)}` }
    : { bg: 'bg-slate-700/40', text: 'text-slate-300', label: 'Extra Polish Paused' };
  ...
  return (
    <div
      ...
      title={`Extra polish estimate. Quality Score=${Number.isFinite(humanIntentIndex) ? Math.round(humanIntentIndex as number) : 'N/A'}. Extra Polish=${rounded >= 0 ? '+' : ''}${rounded}. Confidence=${normalizedConfidence.toFixed(2)}.`}
    >
      {styles.label}
    </div>
  );
