...
import { HUDState } from "../visual-contract";
...
export interface UseActionAuthorityReturn {
  ...
  hudState: HUDState; // Visual projection (FSM → HUD)
  ghost: AAGhost | null; // Current preview overlay (null if not showing)
  holdProgress: number; // 0.0 to 1.0, for visual feedback only
  ...
  policyResult?: PolicyResult; // Level 4 semantic policy evaluation result
...
export function useActionAuthority(context: AAOperationalContext): UseActionAuthorityReturn {
   ...
   *
   * Level 4: Policy checks are performed every 100ms during HOLDING state.
   * If a policy violation is detected, the action auto-expires.
   */
  const arm = useCallback(() => {
    if (!fsmRef.current || !ghost) return;
    if (expireIfContextMismatch()) return;
    ...
    try {
      ...
      // Track last policy check time (avoid checking on every frame)
      let lastPolicyCheckTime = Date.now();

      // Animate progress from 0 to 1 over 400ms
      // LEVEL 4: Integrated policy checking every 100ms
      const updateProgress = () => {
        if (holdTimerRef.current.startTime === null || !fsmRef.current) return;

        const now = Date.now();
        const elapsed = now - holdTimerRef.current.startTime;
        const progress = Math.min(elapsed / 400, 1.0);

        // LEVEL 4: Policy check (every 100ms, not every frame)
        if (now - lastPolicyCheckTime >= 100) {
          lastPolicyCheckTime = now;

          try {
            // Build semantic context from ghost proposal
            ...
            // If policy violation detected: auto-expire
            if (!result.isValid) {
              // Stop animation
              if (holdTimerRef.current.rafId !== null) {
                cancelAnimationFrame(holdTimerRef.current.rafId);
              ...
              // Force FSM to EXPIRED state
              fsmRef.current.transition(AAEvent.EXPIRE, context.contextId);
              setState(fsmRef.current.getState());

              console.warn(
                "[PolicyEngine] Auto-revoked during HOLDING:",
                result.reason,
              );
              return; // Stop animation loop
            }
          } catch (policyError) {
            console.error("[PolicyEngine] Error during policy check:", policyError);
            // Fail-safe: allow action if policy check errors
            // (to prevent DOS via malformed policies)
          }
        }
  ...
  }, [ghost, context.contextId, expireIfContextMismatch]);
  ...
  /**
   * HUDState Projection Adapter
   *
   * Transforms (AAState, holdProgress) → HUDState
   * Logic:
   *   IF state === VISIBLE_GHOST AND holdProgress > 0
   *   THEN hudState = HOLDING
   *   ELSE hudState = state (1:1 passthrough, cast is safe)
   */
  const hudState = useMemo(() => {
    if (state === AAState.VISIBLE_GHOST && holdProgress > 0) {
      return HUDState.HOLDING;
    }
    // Direct 1:1 mapping (AAState names match HUDState names)
    return state as unknown as HUDState;
  }, [state, holdProgress]);
  ...
  return {
    ...
    isExpired,
    isTerminal,
    policyResult, // Level 4: Semantic policy evaluation result
